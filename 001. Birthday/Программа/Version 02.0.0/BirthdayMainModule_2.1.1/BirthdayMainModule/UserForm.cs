...
namespace Birthday
{
    ...
    public void showUserInfo(string name1, string name2, string name3, string team,
                                 ...
                                 string computer, string login, string email, string path)
        {
            ...
            tE_team.Text = team;
            tE_job.Text = job;
            tE_phone1.Text = phone1;
            tE_phone2.Text = phone2;
            tE_dob.Text = dob;
            tE_computer.Text = computer;
            tE_login.Text = login;
            tE_email.Text = email;

            if (path != null)                  // если файл фото существует
            {
                pictureEdit1.LoadAsync(path);  // загрузить фото


                // подогнать размеры изображения:
                pictureEdit1.Image = ScaleImage(pictureEdit1.Image, pictureEdit1.Size.Width, pictureEdit1.Size.Height);

            }
            else                         // если файл фото не существует
            {
                pictureEdit1.Image = ScaleImage(null, pictureEdit1.Size.Width, pictureEdit1.Size.Height);
            }
        }
