...
namespace Birthday
{
    ...
    {
        /*
        Иногда требуется определить такой член класса, который будет использоваться независимо от всех
        остальных объектов этого класса. Как правило, доступ к члену класса организуется посредством 
        объекта этого класса, но в то же время можно создать член класса для самостоятельного применения
        без ссылки на конкретный экземпляр объекта. Для того чтобы создать такой член класса, достаточно 
        указать в самом начале его объявления ключевое слово static.

        Если член класса объявляется как static, то он становится доступным до создания любых объектов
        своего класса и без ссылки на какой-нибудь объект. С помощью ключевого слова static можно
        объявлять как переменные, так и методы. Наиболее характерным примером члена типа static служит
        метод Main(), который объявляется таковым потому, что он должен вызываться операционной системой
        в самом начале выполняемой программы.

        Для того чтобы воспользоваться членом типа static за пределами класса, достаточно указать имя 
        этого класса с оператором-точкой. Но создавать объект для этого не нужно. В действительности
        член типа static оказывается доступным не по ссылке на объект, а по имени своего класса.

        Переменные, объявляемые как static, по существу, являются глобальными. Когда же объекты
        объявляются в своем классе, то копия переменной типа static не создается. Вместо этого все
        экземпляры класса совместно пользуются одной и той же переменной типа static. Такая 
        переменная инициализируется перед ее применением в классе. 
        */

        // поля:
        private static bool f_debugMode;                    // флаги сохранения настроек 
        private static bool f_debugEn;
        private static bool f_birthday;
        private static bool f_holiday;
        private static int f_days;
        private static int f_hours;

        // свойства полей:

        // f_debugMode
        public static bool F_debugMode
        {
            set
            {
                f_debugMode = value;
        ...
        const int cChristian = 7;                          // количество Христианских праздников

        // поля только для чтения:
        readonly string NewL = Environment.NewLine;        // новая строка
        readonly int thisYear = Convert.ToInt32(DateTime.Now.Year.ToString());

        // поля:
        private bool f_sorting = true;                     // флаг переключения сортировки
        private bool f_netConnect = false;                 // флаг наличия подключения 
        private string userName = null;                    // имя сотрудника
        private int notFired = 0;                          // число неуволенных пользователей
        private int teams = 0;                             // число подразделений 
        private int jobs = 0;                              // число профессий
        private int counter = 0;                           // количество праздников
        private int countPass = 0;                         // количество переходных праздников
        private int strOfDobs = 0;                         // строка наличия событий
        private int events = 0;                            // количество событий

        // свойства полей:

        // f_sorting
        public bool F_sorting
        {
            set
            {
              f_sorting = value;
        ...
        private void PrepareToBuildTree()
        {
            ...
            //или кратко foreach (var kvp in array)
            {
                ...
                if (fired == 0)                              // если пользователь не уволен
                {
                    ...
                    for (int i = 0; i < Jobs; i++)
                    {
                        ...
                        {
                            jobStr[index2] = arrOfJobs[i];    // записать в строку название профессии
                            index2++;
                        }
                    }
                }
            ...
            //или кратко foreach (var kvp in array)
            {
                ...
                if (fired == 0)                                              // если пользователь не уволен
                {
                    ...
                    //richTextBox1.Text += arrOfNames[index1] + Convert.ToString(kvp.Value.computer) + NewL;

                    //richTextBox1.Text += arrOfNames[index1] + NewL;        // список ФИО пользователей
                    //richTextBox1.Text += arrOfDobs[index1] + NewL;         // список дней рождений пользователей
                    //richTextBox1.Text += arrOfTeams[index1] + " " + arrOfNames[index1] + NewL;

                    /*
                    // список подразделений пользователей:
                    for(int i=0; i<Teams; i++)
                    {
                        if(arrOfTeams[index1] == keysOfTeams[i])             // если код подразделения пользователя совпадает с ключом подразделения
                        richTextBox1.Text += namesOfTeams[i] + NewL;
                    }
                    */

                    //richTextBox1.Text += jobStr[index1] + NewL;            // список профессий пользователей:
        ...
        public void remind()
        {
            ...
            // очистить массивы:                                                  
            Array.Clear(strOfEvents, 0, MyEvents);             
            Array.Clear(strOfColors, 0, MyEvents);             

            // изменить размерности массивов:
            Array.Resize(ref strOfEvents, 500);
            Array.Resize(ref strOfColors, 500);

            MyEvents = 0;                                      // количество событий
            StrOfDobs = 0;
            ...
            if (F_birthday)                                  // если выводить дни рождения
            {
                ...
                {
                    strOfEvents[MyEvents] = "\n\n\n\nНедавние дни рождения: \n\n";  // заголовок
                    strOfColors[MyEvents] = Color.Blue;                     // цвет заголовка
                    MyEvents++;

                    for (int j = 0; j < numbOfDobsLately; j++)
                    {
                        strOfEvents[MyEvents] = s1[0, j];                   // ФИО
                        strOfColors[MyEvents] = Color.Red;
                        MyEvents++;

                        for (int i = 1; i < 8; i++)
                        {
                            strOfEvents[MyEvents] = s1[i, j];               // данные
                            strOfColors[MyEvents] = Color.Black;
                            MyEvents++;
                        }
                    }
                }
                ...
                {
                    strOfEvents[MyEvents] = "Сегодня день рождения: \n\n";  // заголовок
                    strOfColors[MyEvents] = Color.Blue;                     // цвет заголовка
                    MyEvents++;

                    for (int j = 0; j < numbOfDobs; j++)
                    {
                        strOfEvents[MyEvents] = s2[0, j];                   // ФИО
                        strOfColors[MyEvents] = Color.Red;
                        MyEvents++;

                        for (int i = 1; i < 8; i++)
                        {
                            strOfEvents[MyEvents] = s2[i, j];               // данные
                            strOfColors[MyEvents] = Color.Black;
                            MyEvents++;
                        }
                    }
                ...
                if (numbOfDobsSoon > 0)                                   // если есть ближайшие дни рождения
                {
                    ...
                    {
                        case 1:
                            strOfEvents[MyEvents] = "Дни рождения в ближайший день: \n\n";
                            break;

                        case 2:
                        case 3:
                        case 4:
                            strOfEvents[MyEvents] = "Дни рождения в ближайшие " + f_days + " дня: \n\n";
                            break;

                        case 5:
                        case 6:
                        case 7:
                            strOfEvents[MyEvents] = "Дни рождения в ближайшие " + f_days + " дней: \n\n";
                            break;
                    }
                    ...
                    {
                        strOfEvents[MyEvents] = s3[0, j];                   // ФИО
                        strOfColors[MyEvents] = Color.Red;
                        MyEvents++;

                        for (int i = 1; i < 8; i++)
                        {
                            strOfEvents[MyEvents] = s3[i, j];               // данные
                            strOfColors[MyEvents] = Color.Black;
                            MyEvents++;
                        }
                    }
            ...
            if (f_holiday)
            {
                ...
                {
                    strOfEvents[MyEvents] = "Сегодня даты: \n\n";              // заголовок
                    strOfColors[MyEvents] = Color.Blue;                        // цвет заголовка
                    MyEvents++;

                    for (int j = 0; j < numbOfHolidays; j++)
                    {
                        strOfEvents[MyEvents] = arrOfHolidaysToday[j] + "\n";  // данные
                        strOfColors[MyEvents] = Color.Black;
                        MyEvents++;
                    }

                    strOfEvents[MyEvents - 1] += "\n";
                }
                ...
                if (numbOfHolidaysSoon > 0)                               // если есть ближайшие дни рождения
                {
                    ...
                    {
                        case 1:
                            strOfEvents[MyEvents] = "Даты в ближайший день: \n\n";
                            break;

                        case 2:
                        case 3:
                        case 4:
                            strOfEvents[MyEvents] = "Даты в ближайшие " + f_days + " дня: \n\n";
                            break;

                        case 5:
                        case 6:
                        case 7:
                            strOfEvents[MyEvents] = "Даты в ближайшие " + f_days + " дней: \n\n";
                            break;
                    }
                    ...
                    {
                        strOfEvents[MyEvents] = arrOfHolidaysSoon[j] + "\n";  // данные
                        strOfColors[MyEvents] = Color.Black;
                        MyEvents++;
                    }
                }
            ...
            // изменить размерности массивов:
            Array.Resize(ref strOfEvents, MyEvents);
            Array.Resize(ref strOfColors, MyEvents);
        ...
        public void ShowRemindForm()
        {
            ...
            if (StrOfDobs != 0)
            {
                ...
                else
                {
                    ...
                    remindForm.ShowRemind(strOfEvents, strOfColors, MyEvents);  // вывести сообщение
                }
            }
        }
                    ...
                    if (tV_Tree.SelectedNode.FirstNode == null)        // если нет дочерних узлов
                    {
                        ...
                        //или кратко foreach (var kvp in array)
                        {
                            ...
                            if (fired == 0)                            // если пользователь не уволен
                            {
                                ...
                                string teamStr = null;                 // строка для названия подразделения
                                string jobStr = null;                  // строка для названия профессии

                                // записать название подразделения в строку:
                                for (int i = 0; i < Teams; i++)
                                {
                                    if (keysOfTeams[i] == team)        // если ключ подразделения равен идентификатору подразделения
                                        teamStr = namesOfTeams[i];     // записать в строку название подразделения
                                }
                                ...
                                {
                                    if (arrOfKeyJobs[i] == job)        // если ключ профессии равен идентификатору профессии
                                        jobStr = arrOfJobs[i];         // записать в строку название профессии
                                }
                                ...
                                if (tV_Tree.SelectedNode.Text == name1 + " " + name2 + " " + name3)
                                {
                                    ...
                                    {
                                        UserForm.showUserInfo(name1, name2, name3, teamStr, jobStr, phone1, phone2, dob, computer, login, email, pathToPhoto);
                                    }
                                    else
                                    {
                                        UserForm.showUserInfo(name1, name2, name3, teamStr, jobStr, phone1, phone2, dob, computer, login, email, null);
                                    }
                                }
                            }
                        }
        ...
        private void usersGet()
        {
            ...
            try
            {
                ...
                /*
                // отсортировать пользователей по дню рождения в алфавитном порядке 
                usersGetArray = usersGetArray.OrderBy(pair => pair.Value.dob.Substring(5,5)).ToDictionary(pair => pair.Key, pair => pair.Value);
                
                // отсортировать пользователей по номеру компьютера в алфавитном порядке 
                usersGetArray = usersGetArray.OrderBy(pair => pair.Value.computer).ToDictionary(pair => pair.Key, pair => pair.Value);
                */
            }
            catch (Exception ex)
            {
        ...
        private void authStart()
        {
            ...
            if (F_netConnect)
            {
                ...
                try
                {
                    //richTextBox1.Text = pathToServer.ToString();
