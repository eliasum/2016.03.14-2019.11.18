...
$_SESSION['hide_checked']     = (int) isset($_POST['hideVal']); 

// записываем в сессию поисковые запросы
$_SESSION['nameReq']          =             $_POST['nameField']; 
$_SESSION['codeReq']          =             $_POST['codeField']; 
$_SESSION['kspReq']           =             $_POST['kspField']; 
 
$checkboxes = array(
// записываем в переменную текущее состояние галочки, если есть сессия и она равна 1, то записываем true, если нет - false
  ...
  'hideInd'     => (isset($_SESSION['hide_checked'])     && $_SESSION['hide_checked']     == 1 ? true : false), 
  
// записываем в переменные поисковые запросы
  'nameR'       => (      $_SESSION['nameReq']),  
  'codeR'       => (      $_SESSION['codeReq']),  
  'kspR'        => (      $_SESSION['kspReq']),  
);
...
//---------------------------------------------------->  
// вывести дату последнего изменения остатков:
  $query1 = " SELECT `date`
              FROM `equips`
		      ORDER BY `id` DESC LIMIT 1";                      // запрос к БД
		   
  $result1 = mysql_query($query1);                              // результат запроса
   
  $row1 = mysql_fetch_array($result1);                          // записать результат запроса в переменную $row как массив
    
  if(mysql_num_rows($result1) > 0)                              // если количество рядов результата запроса больше нуля
  {
    $lastDate = " Последнее изменение: от " . $row1['date'];
  }
  else 
  {
    $lastDate = "";
  ...
  if(mysql_num_rows($result) > 0)                               // если количество рядов результата запроса больше нуля
  {
    ...
    "<table> 
       <tr>                                                        
         ...
         <th class='row7'><b>Дата первого ввода</b></th>
...
//---------------------------------------------------->			 
      ...
      {	
	    echo		                                            // открыть доступ к действиям
         '<td>
	       <div class="links">
	         <a href="put.php?id='     . $row['id'] . '">Оприходовать</a>  
             <a href="take.php?id='    . $row['id'] . '">Расходовать</a>
		     <!--<a href="editing.php?id=' . $row['id'] . '">Редактировать</a>-->
           </div>
	     </td>';
	  }
//---------------------------------------------------->	  
	  ...
	  if(checkbox_verify('hideVal') && ($row['count']!=0))
	  {
      echo                                                      // открыть доступ к действиям
      "<tr>
         <td>" . $row['id'] . "</td>                            
...
<head>
  ...
  <title>Кладовка КБА. Остаток. (Начальная страница)</title>
</head>

<body>

<p><b>Кладовка КБА. Остаток. (Начальная страница) <?php echo $lastDate ?></b></p>
...
?>
  ...
  <a href="turnover.php">Оборотная ведомость</a> 
  <a href="WebHelp/manual.htm">Руководство</a> 
</div>
...
<div style="float: auto;" >
  <fieldset id="block0" name="sortingGroup" style="border:0px solid grey; display:inline;">
    <legend><b>Скрыть наименования с нулевым количеством:</b></legend>
      <div class="fieldgroup">
	    <input id='idHide' type="checkbox" <?=($checkboxes['hideInd'] ? 'checked' : null); ?> name="hideVal" /><!--  onClick="timeout(this)" -->
		<label for='idHide'> Скрыть</label>
     </div>	
	</legend>
  </fieldset>
</div>
...
<div style="float: left;"  >
  <fieldset id="block2" name="orderGroup" style="border:0px solid grey; display:inline;">
	...
	<div class="fieldgroup" >
	  <input type="submit" name="setButton" value="Отсортировать" onClick="clearAllFields()"/>
	</div>
...
<div>
  <fieldset id="block3" name="searchGroup" style="border:0px solid grey; display:inline;">
    <label><b>Поиск совпадения по:</b></legend>  
	<table>
	  <tr>
	    <td class="search" align="left"><label for='search1'> Наименованию:</label></td>
		<td class="search"><input style="margin-left: 3em;" type="text" name="nameField" id="search1" value="<?php echo $checkboxes['nameR']; ?>"></td>
		<td class="search"><input style="margin-left: 3em;" type="submit" name="showName" value="Показать" onClick="clear1Fields()" ></td>
	  </tr>
	  <tr>
	    <td class="search" align="left"><label for='search2'> Коду: </label></td>
		<td class="search"><input style="margin-left: 3em;" type="text" name="codeField" id="search2" value="<?php echo $checkboxes['codeR']; ?>"></td>
		<td class="search"><input style="margin-left: 3em;" type="submit" name="showCode" value="Показать" onClick="clear2Fields()" ></td>
	  </tr>
	  <tr>
	    <td class="search" align="left"><label for='search3'> Месту хранения: </label></td>
		<td class="search"><input style="margin-left: 3em;" type="text" name="kspField" id="search3" value="<?php echo $checkboxes['kspR']; ?>"></td>
		<td class="search"><input style="margin-left: 3em;" type="submit" name="showKsp" value="Показать" onClick="clear3Fields()" ></td>
	  </tr>
	</table>
  </fieldset>
</div>	
...
{
  $name = strip_tags(trim($_POST['nameField']));                // что ищем?
...
{
  $code = strip_tags(trim($_POST['codeField']));                // что ищем? 
...
{
  $ksp = strip_tags(trim($_POST['kspField']));                  // что ищем? 
