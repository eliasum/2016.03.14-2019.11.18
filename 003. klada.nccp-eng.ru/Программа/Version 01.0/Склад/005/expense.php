...
*/
  ...
  $sql = "SELECT *  
		  ...
		  ORDER BY id ASC";  // запрос на выбор всех полей из таблицы expenses со значениями поля date от $begin до $end, сортировать по id
...
<?php
  ...
  $sql = "SELECT *
          FROM expenses
          ORDER BY id ASC";   // запрос на выбор всех полей из таблицы expenses, сортировать по id
  
  $result = mysql_query($sql);                                      // результат запроса записать в переменную $result

  mysql_close();                                                    // закрыть соединение с БД

  //$row = mysql_fetch_array($result);                              // записать таблицу expenses в переменную $row как массив
...
?>
      <tr>
	    ...
	    <td><?php echo $row['date'];?></td>
		<td>
		  <a href="note.php?id=<?php echo $row['id']?>">Доб. прим.</a><br />
		  <?php echo $row['note'];?>
		</td>
      </tr>
<?php 
