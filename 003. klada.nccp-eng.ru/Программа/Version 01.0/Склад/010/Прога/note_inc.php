...
global $note_inc;                                            // глобальные переменные
...
$result = mysql_query(" SELECT * 
                        FROM incomes
                        WHERE id='$id'
					  ");                                    // запрос на выбор всех полей из таблицы incomes с id='$id'
...
if(isset($_POST['savenote_inc']))                            // если кнопка 'savenote_inc' была нажата
{
  ...
  */
  ...
  $note_inc = strip_tags(trim($_POST['note_inc']));          // $_POST - глобальный массив, ввод в переменные с обработкой
	
  if($note_inc!=null)                                        // если поле 'примечание' не пустое 
  {
	// далее подлючаемся к бд:
	mysql_query(" 
				UPDATE incomes 
				SET note_inc='$note_inc'
				WHERE id='$id' 
				");  // запрос на обновление полей таблицы incomes, установить поле note_inc с id='$id' значением переменной $note_inc
...
<br/>Примечание<br/>
<?php
if($note_inc!=null)  // если примечание было изменено
{
?>
<input type="text" name="note_inc" value="<?php echo $note_inc; // записать в поле отредактированное значение ?>" /><br/><br/>
<?php
}
else
{
?>
<input type="text" name="note_inc" value="<?php echo $row['note_inc'] // записать в поле значение из БД ?>" /><br/><br/>  
<?php  
}
?>
