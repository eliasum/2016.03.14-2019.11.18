...
<head>
  ...
  <title>Кладовка КБА. Расход.</title>
</head>
<body>

<p><b>Кладовка КБА. Расход.</b></p>
  ...
  if(mysql_num_rows($result) > 0)                               // если количество рядов результата запроса больше нуля
  {
    ...
    "<table> 
       <tr>                                                        
         ...
         <th class='row17'><b>Дата последнего расхода</b></th>
...
//---------------------------------------------------->	 
	     if($authorization!=0)                                           // проверка пользователя
         {	
		   echo '<a href="note_exp.php?id=' . $row['id'] . '">Доб. прим.</a>';
	     }
//---------------------------------------------------->
		 
		 echo "<br />" . $row['note_exp'] .
		 "</td>
       </tr>";	   
    }
    echo
...
*/
  ...
  // запрос на выбор всех полей из таблицы expenses со значениями поля date от $begin до $end, сортировать по id
  $query = "SELECT *  
            FROM `expenses` 
...
{                                                                  
  // запрос на выбор всех полей из таблицы expenses, сортировать по id
  $query = "SELECT *
            FROM `expenses`
            ORDER BY `id` ASC";   
